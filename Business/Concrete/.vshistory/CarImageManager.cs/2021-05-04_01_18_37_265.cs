...
namespace Business.Concrete
{
    public class CarImageManager : ICarImageService
    {
        ...
        public IDataResult<List<CarImages>> GetImagesByCarId(int id)
        {
            ...
            return new SuccessDataResult<List<CarImages>>(CheckIfCarImageNull(id).Data, "Messages.ImagesListed");
        }

        public IResult Update(IFormFile file, CarImages carImage)
        {
            var isImage = _iCarImageDal.Get(c => c.ImageId == carImage.ImageId);
            if (isImage == null)
            {
                return new ErrorResult("Messages.ImageNotFound");
            }
